...
/**
 * User: mew <p />
 * Time: 17/11/7 09:34  <p />
 * Version: V1.0  <p />
 * Description: Redis 工具类 <p />
 */
public class RedisUtil {
    ...
    /** Redis服务器IP */
    private static String IP = PropertiesFileUtil.getInstance("application").get("master.redis.ip");

    /** Redis的端口号 */
    private static int PORT = PropertiesFileUtil.getInstance("application").getInt("master.redis.port");

    /** 访问密码 */
    private static String PASSWORD = AESUtil.AESDecode(PropertiesFileUtil.getInstance("application")
            .get("master.redis.password"));

    /** 可用连接实例的最大数目，默认值为8；如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽) */
    private static int MAX_ACTIVE = PropertiesFileUtil.getInstance("application").getInt("master.redis.max_active");

    /** 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8 */
    private static int MAX_IDLE = PropertiesFileUtil.getInstance("application").getInt("master.redis.max_idle");

    /** 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出 JedisConnectionException */
    private static int MAX_WAIT = PropertiesFileUtil.getInstance("application").getInt("master.redis.max_wait");

    /** 超时时间 */
    private static int TIMEOUT = PropertiesFileUtil.getInstance("application").getInt("master.redis.timeout");
    ...
    // redis过期时间,以秒为单位
    /** 一小时 */
    public final static int EXRP_HO1UR = 60 * 60;
