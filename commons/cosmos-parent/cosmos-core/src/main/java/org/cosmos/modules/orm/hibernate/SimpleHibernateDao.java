package org.cosmos.modules.orm.hibernate;

import org.apache.commons.lang.StringUtils;
import org.cosmos.modules.orm.MatchType;
import org.cosmos.modules.utils.reflection.ReflectionUtils;
...
/**
 * User: mew <p />
 * Time: 17/11/6 15:51  <p />
 * Version: V1.0  <p />
 * Description:
 * 封装Hibernate原生API的DAO泛型基类. <br/>
 * 可在Service层直接使用, 也可以扩展泛型DAO子类使用, 见两个构造函数的注释. <br/>
 * 参考Spring2.5自带的Petlinc例子, 取消了HibernateTemplate, 直接使用Hibernate原生API. <br/>
 *
 * @param <T>
 *         DAO操作的对象类型
 * @param <PK>
        ...
        switch (matchType) {
            ...
            case LIKE:
                if (propertyValue instanceof String) {
                    ...
                    logger.warn("属性{}使用LIKE查询，为非字符型，实际类型为：{}，自动使用EQ查询", propertyName,
            ...
            case LIKEE:
                if (propertyValue instanceof String) {
                    ...
                    logger.warn("属性{}使用LIKEE查询，为非字符型，实际类型为：{}，自动使用EQ查询", propertyName,
                            propertyValue.getClass().getName());
                    criterion = Restrictions.eq(propertyName, propertyValue);
                }
                break;
            case LIKES:
                if (propertyValue instanceof String) {
                    ...
                    logger.warn("属性{}使用LIKES查询，为非字符型，实际类型为：{}，自动使用EQ查询", propertyName,
                            propertyValue.getClass().getName());
                    criterion = Restrictions.eq(propertyName, propertyValue);
                }
                break;
