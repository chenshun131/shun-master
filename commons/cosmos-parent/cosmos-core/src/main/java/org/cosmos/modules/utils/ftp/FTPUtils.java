package org.cosmos.modules.utils.ftp;
...
public final class FTPUtils {
    ...
    private FTPUtils() {
        super();
        // 做调试用的日志
        ftpLog.info("FTPUtils instance created:" + this);
    }
    ...
    private FTPClient connect(FTPClient client) throws SocketException, IOException {
        if (client == null) {
            ...
            ftpLog.info("FTPUtils对象中的连接方法中创建ftp client对象引用成功[FTPUtils inst addr:" + this +
                    "; ftp client inst addr:" + client + "]");
        }
    ...
    public void closeFTPClient() {
        try {
            ...
            if (ftpClient.isConnected()) {
                ...
                ftpLog.info("断开FTPUtils对象中处于连接状态的ftp client对象引用[FTPUtils inst addr:" + this + "; ftp client inst " +
                        "addr:" + ftpClient + "]");
                ftpClient = null;
            } else {
                ftpLog.info("FTPUtils对象中的ftp client对象引用未处于连接状态[FTPUtils inst addr:" + this + "; ftp client inst " +
                        "addr:" + ftpClient + "]");
            }
        } catch (Exception e) {
