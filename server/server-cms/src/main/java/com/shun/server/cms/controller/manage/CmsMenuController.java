...
public class CmsMenuController extends BaseController {
    ...
    public Object list(@RequestParam(required = false, defaultValue = "0", value = "offset") int offset,
        ...
        List<CmsMenuPo> rows = cmsMenuService.selectByExampleForOffsetPage(cmsMenuExample, offset, limit);
    ...
    @ResponseBody
    public Object create(CmsMenuPo cmsMenu) {
    ...
    public String update(@PathVariable("id") int id, ModelMap modelMap) {
        CmsMenuPo menu = cmsMenuService.selectByPrimaryKey(id);
    ...
    @ResponseBody
    public Object update(@PathVariable("id") int id, CmsMenuPo cmsMenu) {
        ComplexResult result = FluentValidator.checkAll()
                .on(cmsMenu.getName(), new LengthValidator(1, 20, "名称"))
                .doValidate()
                .result(ResultCollectors.toComplex());
    ...
    public Object up(@PathVariable("id") int id) {
        CmsMenuPo cmsMenu = cmsMenuService.selectByPrimaryKey(id);
        if (null == cmsMenu) {
            return new CmsResult(CmsResultConstant.INVALID_PARAMETER, "无效参数！");
        }
        CmsMenuPoExample cmsMenuExample = new CmsMenuPoExample();
        CmsMenuPoExample.Criteria criteria = cmsMenuExample.createCriteria();
        ...
        CmsMenuPo upCmsMenu = cmsMenuService.selectFirstByExample(cmsMenuExample);
        if (null == upCmsMenu) {
