...
import com.shun.server.util.SerializableUtil;
...
/**
 * User: mew <p />
 * Time: 17/11/9 08:57  <p />
 * Version: V1.0  <p />
 * Description: 基于redis的sessionDao，缓存共享session 以实现会话的存储和持久化，需要的包 org.apache.shiro －> shiro-ehcache <p />
 */
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    protected void doDelete(Session session) {
        ...
        if ("server".equals(upmsType)) {
            ...
            _log.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(ZHENG_UPMS_CLIENT_SESSION_IDS + "_"
                    + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
