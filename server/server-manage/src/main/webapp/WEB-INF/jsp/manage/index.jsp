...
<head>
	...
	<link href="${basePath}/static/plugins/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css" rel="stylesheet"/>
			...
			<ul class="hi-menu">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu dm-icon pull-right">
						...
						<c:forEach var="upmsSystem" items="${upmsSystems}">
						<li>
							<a class="waves-effect switch-systems" href="javascript:;" systemid="${upmsSystem.systemId}" systemname="${upmsSystem.name}" systemtitle="${upmsSystem.title}">
								<i class="${upmsSystem.icon}"></i> ${upmsSystem.title}
							</a>
						</li>
						</c:forEach>
					</ul>
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu dm-icon pull-right">
						<li class="hidden-xs">
							<a class="waves-effect" data-ma-action="fullscreen" href="javascript:fullPage();">
								<i class="zmdi zmdi-fullscreen"></i> 全屏模式
							</a>
						</li>
						<li>
							<a class="waves-effect" data-ma-action="clear-localstorage" href="javascript:;">
								<i class="zmdi zmdi-delete"></i> 清除缓存
							</a>
						</li>
						<li>
					...
					<ul>
						<c:forEach var="subUpmsPermission" items="${upmsPermissions}">
							<c:if test="${subUpmsPermission.pid == upmsPermission.permissionId}">
								...
								<li>
									<a class="waves-effect" href="javascript:Tab.addTab('${subUpmsPermission.name}', '${systemBasePath}${subUpmsPermission.uri}');">${subUpmsPermission.name}</a>
								</li>
							</c:if>
						</c:forEach>
					</ul>
...
<script src="${basePath}/static/plugins/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js"></script>
<script src="${basePath}/static/plugins/BootstrapMenu.min.js"></script>
<script src="${basePath}/static/plugins/device.min.js"></script>
<script src="${basePath}/static/plugins/jquery.cookie.js"></script>
<script src="${basePath}/static/js/admin.js"></script>
<script src="${basePath}/static/plugins/fullPage/jquery.fullPage.min.js"></script>
<script src="${basePath}/static/plugins/fullPage/jquery.jdirk.min.js"></script>
</body>
</html>
