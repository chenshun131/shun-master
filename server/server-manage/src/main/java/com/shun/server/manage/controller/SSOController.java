...
import com.shun.server.shiro.session.UpmsSession;
...
/**
 * User: mew <p />
 * Time: 17/11/9 14:24  <p />
 * Version: V1.0  <p />
 * Description: 单点登录管理 <p />
 */
@Controller
@RequestMapping("/sso")
...
public class SSOController extends BaseController {
    ...
    public String index(HttpServletRequest request) throws Exception {
        ...
        upmsSystemPoExample.createCriteria().andNameEqualTo(appid);
        int count = upmsSystemService.countByExample(upmsSystemPoExample);
        if (0 == count) {
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        if (StringUtils.isBlank(hasCode)) {
            // 使用shiro认证
            CustomToken token = new CustomToken(username, password, LoginType.ADMIN);
            try {
            ...
            // 全局会话的code
            RedisUtil.set(ZHENG_UPMS_SERVER_SESSION_ID + "_" + sessionId, code,
